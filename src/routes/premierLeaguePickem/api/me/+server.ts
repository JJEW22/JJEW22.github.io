...
// Restores the logged-in user, their fan team, saved picks, and season-prediction
// lock state on load.
export const GET: RequestHandler = async ({ locals }) => {
    if (!locals.user) return json({ user: null });
    const me = (await sql`select fan_team, predictions_saved_at from users where id = ${locals.user.id}`)[0];
